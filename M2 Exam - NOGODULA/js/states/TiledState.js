...
Platformer.TiledState.prototype.init = function (level_data) {
    ...
    // Create map and Set the tileset
...
Platformer.TiledState.prototype.create = function () {
    ...
    // Add and Create Map layers
    this.layers = {};
    this.map.layers.forEach(function (layer) {
        this.layers[layer.name] = this.map.createLayer(layer.name);
        if (layer.properties.collision) { // this is collision layer
            collision_tiles = [];
            layer.data.forEach(function (data_row) { // this is for find tiles used in the layer
                data_row.forEach(function (tile) {
                    // just check if it's a valid tile index and isn't already in the list
...
Platformer.TiledState.prototype.restart_level = function () {
    ...
    // Checkpoint function, if the player reach the checkpoint
    // the player automatice respawn at the checkpoint flag
